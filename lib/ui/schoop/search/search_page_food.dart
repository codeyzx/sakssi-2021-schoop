...
class SearchPageFood extends StatefulWidget {
  const SearchPageFood({Key? key}) : super(key: key);

  @override
  _SearchPageFoodState createState() => _SearchPageFoodState();
}

class _SearchPageFoodState extends State<SearchPageFood> {
  ...
  void onSearch(String data) async {
    CollectionReference food = _firestore.collection('food');
    print('SEARCH : $data');
    if (data.length == 0) {
      queryAwal.value = [];
      tempSearch.value = [];
      ...
      if (queryAwal.length == 0 && data.length == 1) {
        final keyNameResult = await food
  ...
  Widget build(BuildContext context) {
    CollectionReference hfood = _firestore.collection('food');
    return Builder(
