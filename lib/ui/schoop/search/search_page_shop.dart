...
class SearchPageShop extends StatefulWidget {
  const SearchPageShop({Key? key}) : super(key: key);

  @override
  _SearchPageShopState createState() => _SearchPageShopState();
}

class _SearchPageShopState extends State<SearchPageShop> {
  ...
  void onSearch(String data) async {
    CollectionReference shop = _firestore.collection('shop');
    print('SEARCH : $data');
    if (data.length == 0) {
      queryAwal.value = [];
      tempSearch.value = [];
      ...
      if (queryAwal.length == 0 && data.length == 1) {
        final keyNameResult = await shop
